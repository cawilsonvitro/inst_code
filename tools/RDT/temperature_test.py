...
class NI_RDT_temp_test():
    ...
    """
    #region init
    def __init__(self, T_bias_on, t_run, t_delay,fan_delay, T_cool, num_of_meas, min_val, max_val):
        """
    ...
    def maintain_temp(self,tolerance = 5 ):
        """
        ...
        """
        #need to make better 
        t_upper = self.T_bias + tolerance
        t_lower = self.T_bias - tolerance
        print(t_upper, t_lower, self.Temp_1)
        
        if self.Temp_1 < t_lower:
            print("triggered heating")
            self.Relay_Controller.write([True, True, False])
        
        if t_lower < self.Temp_1 < t_upper:
            print("triggered maintaining")
            # self.Relay_Controller.write([False, True, False])
        
        if self.Temp_1 > t_upper:
            print("triggered cooling")
    ...
    def standard_procedure(self):
        ...
        while N <= self.N_meas:
            self.maintain_temp(tolerance = 5)  # Adjust tolerance as needed
            temp:list[float|str] = []
...
if __name__ == "__main__":
    try:
        rdt:NI_RDT_temp_test = NI_RDT_temp_test(150, 1, 1, 30, 30, 60, -.05, .05)
        rdt.init_rdt()
        t.sleep(1)  # Allow time for initialization
